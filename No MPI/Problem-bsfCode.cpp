	...
	*success = MTX_LoadPoint(PD_x, PP_MTX_POSTFIX_Z0);
	...
	if (!PointBelongsPolytope(PD_x, PP_EPS_ZERO)) {
		if (BSF_sv_mpiRank == BSF_sv_mpiMaster)
			cout << "Starting point does not belong to the feasible polytope!!!\n";
		*success = false;
		return;
	}
	if (Vector_Norm(PD_c) < PP_EPS_ZERO) {
		if (BSF_sv_mpiRank == BSF_sv_mpiMaster)
			cout << "Norm of vector c is equal to zero!!!\n";
		*success = false;
		return;
	}

	ApexPoint(PD_x, PD_apexPoint);
}
...
void PC_bsf_ParametersOutput(PT_bsf_parameter_T parameter) {
	...
	cout << "x0 on hyperplanes: "; Print_IncludingHyperplanes(PD_x, PP_EPS_ZERO);
#endif // PP_DEBUG
...
void PC_bsf_ProblemOutput(PT_bsf_reduceElem_T* reduceResult, int reduceCounter, PT_bsf_parameter_T parameter, double t) {
	...
	cout << "Number of including hyperplanes: " << Number_IncludingHyperplanes(parameter.x, PP_EPS_ON_HYPERPLANE) << endl;
	cout << "u0 on hyperplanes: "; Print_IncludingHyperplanes(parameter.x, PP_EPS_ON_HYPERPLANE);
...
namespace SF {
	...
	static inline void Print_IncludingHyperplanes(PT_vector_T x, double eps) {
		bool comma = false;
...
namespace PF {
	...
	inline void ApexPoint(PT_vector_T innerPont, PT_vector_T apexPoint) {
		...
		for (int i = 0; i < PD_m; i++)
			if (!PD_recessive_tag && PointBelongsHalfspace_i(apexPoint, i, PP_EPS_ZERO)) {
				if (PointBelongsHyperplane_i(apexPoint, i, PP_EPS_ZERO))
					continue;
				cout << "Apex point inside recessive half-space " << i << "!" << endl;
			}

		cout << "Number of including hyperplanes: " << Number_IncludingHyperplanes(apexPoint, PP_EPS_ON_HYPERPLANE) << endl;
		cout << "Apex point belongs to hyperplane: "; Print_IncludingHyperplanes(apexPoint, PP_EPS_ZERO); cout << endl;
#endif // PP_DEBUG
